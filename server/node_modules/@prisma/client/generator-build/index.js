...
function getArgs(modelName, actionName) {
  ...
  return `args${actionName === DMMF.ModelAction.findMany || actionName === DMMF.ModelAction.findFirst || actionName === DMMF.ModelAction.deleteMany || actionName === DMMF.ModelAction.createMany || actionName === "findFirstOrThrow" || actionName === "findUniqueOrThrow" ? "?" : ""}: SelectSubset<T, ${getModelArgName(modelName, actionName)}${ifExtensions("<ExtArgs>", "")}>`;
}
...
var PayloadType = class {
  ...
  toTS() {
    ...
    const findManyArg = isModel && this.findMany ? ` | ${getModelArgName(name, DMMF.ModelAction.findMany)}${ifExtensions("<ExtArgs>", "")}` : "";
  ...
  ? ${ifExtensions(`_${name}`, name)} ${include.length > 0 ? ifExtensions(`& runtime.Types.Utils.EmptyToUnknown<${include}>`, ` & ${include}`) : ""}
...
`;
    ...
    return `${availableActions.includes(DMMF.ModelAction.aggregate) ? `type ${countArgsName}${ifExtensions("<ExtArgs extends runtime.Types.Extensions.Args = never>", "")} = Merge<
...
// src/generation/TSClient/PrismaClient.ts
function Omit(O2, K2) {
  return `{ [P in keyof ${O2} as P extends ${K2} ? never : P]: ${O2}[P] }`;
}
__name(Omit, "Omit");
function Pick(O2, K2) {
  return `{ [P in keyof ${O2} as P extends ${K2} ? P : never]: ${O2}[P] }`;
}
__name(Pick, "Pick");
...
__name(Patch, "Patch");
function clientExtensionsModelResultDefinition() {
  const modelNames = Object.keys(this.dmmf.getModelMap());
  const modelResultNeedsGenericParam = /* @__PURE__ */ __name((modelName) => {
  ...
  }, "modelResultNeedsGenericParam");
  const modelResultFieldsGenericParam = /* @__PURE__ */ __name((modelName) => {
...
__name(clientExtensionsModelResultDefinition, "clientExtensionsModelResultDefinition");
...
function clientExtensionsQueryDefinition() {
  ...
  const params = `{${modelNames.reduce((acc, modelName) => {
    ...
    return `${acc}
      ${lowerCase(modelName)}?: {${actions.reduce((acc2, action) => {
      return `${acc2}
...
function clientExtensionsDefinition() {
  ...
  const definition = /* @__PURE__ */ __name(() => `
  /**
...
__name(clientExtensionsDefinition, "clientExtensionsDefinition");
...
get ${methodName}(): ${ifExtensions(
          `${Patch(
            `(ExtArgs['model'] & {})['${lowerCase(m2.model)}'] & (ExtArgs['model'] & {})['$allModels']`,
            `Prisma.${m2.model}Delegate<GlobalReject, ExtArgs>`
          )}`,
